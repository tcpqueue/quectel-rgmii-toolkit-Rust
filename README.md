...
设备助手采用原生 **WinUI 3** 中文界面，支持系统深浅色主题、自动检测、多设备选择、分步进度、实时日志、故障诊断、打开管理页面和查看报告。面向 **Windows 10 2004（19041）及以上 / Windows 11，x64**；截图使用模拟设备。
...
ADB 尚未开启时，进入设备助手的 **① 连接与准备**：

1. 刷新串口，选择 **Quectel USB AT Port**，点击 **识别模块**。请关闭占用串口的其他工具；串口驱动仍需按设备安装。
2. 点击 **检查并解锁 ADB**。先读取 USB 配置的倒数第二项，值为 **1 或 2** 时直接跳过解锁；仅在值为 **0** 时显示待执行指令，确认后计算密钥并启用 ADB。
3. 工具保留当前 VID、PID 和其他 USB 接口设置，检查密钥响应并复查配置。若需要重启，单独点击 **重启模块** 并确认；不会自动重启。
4. 进入 **② 安装与维护**，刷新并选择 ADB 设备，继续安装。已有 ADB 连接可直接跳过准备页。

![移远高通连接准备](docs/images/windows-prepare.png)

需要设置 RJ45 时，展开 **网口配置**，选择 RTL8125 / RTL8168 及转接板需要的数据接口方案，预览指令后执行。默认提供 RM520N 的 PCIe + MPDN NAT 方案；参考工具兼容方案会额外修改 USB 网卡、SIM 检测和自动拨号，需按转接板要求选择。

**设备信息**手动读取 SIM 状态、信号、温度、APN、频段和接口配置，不持续轮询。**高级 AT 工具**支持最多 32 条逐行指令、常用查询和本机保存；遇到错误、超时或停止请求即中止后续指令，已完成的修改不会自动回滚。恢复出厂单独确认，不能当作卸载使用。

准备页仅开放已列入适配范围的移远高通型号：RM500Q、RM502Q、RM520N、RM521F、RG500Q、RG502Q、RG520N、RG520F、RG521F 系列，USB 配置要求 VID 2C7C、PID 0800/0801。这是协议检查范围，不代表所有型号均已实机验证；紫光展锐、MTK 和其他厂商不套用这些配置。日志默认仅在电脑内存，手动保存才写入电脑；不记录解锁密钥，不提供短信发送或删除指令。
...
移远高通准备流程参考用户提供的 ADBUnlockTool v1.33 中的指令与密钥算法，用 C# 独立实现；未打包或分发原 Python 工具。
