...
历史数据采用固定长度环形数组和定点数，两类历史的原始数组合计不超过 **6,240 字节**。AT 队列最多 16 个请求，缓存最多 64 项、响应内容合计最多 1 MiB，会话与 WebSocket 也有数量限制。这些是内部缓冲区上限，不代表程序总内存占用。
...
RM520N-EU 同条件后台采样中，CPU 约 **1.46% → 0.45%**，PSS 内存约 **7.25 MiB → 1.40 MiB**；正常页面访问场景下 Rust 约 **3.1 MiB**。ARM 程序从 7.93 MB 缩小到 4.15 MB。采样条件与适用范围见 [验证记录](docs/validation.md)。
